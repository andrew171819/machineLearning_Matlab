...
net{2}.layers = {} ;
net{2}.layers{end + 1} = struct('type', 'mlp', 'weights', {{f * randn(n_cell_nodes, n_hidden_nodes + n_input_nodes, 'single'), zeros(n_cell_nodes, 1, 'single')}});
...
net{4}.type = 'Fit';
net{4}.layers = {};
net{4}.layers{end + 1} = struct('type', 'mlp', 'weights', {{f * randn(n_output_nodes, n_hidden_nodes, 'single'), zeros(n_output_nodes, 1, 'single')}});
