...
net{1}.layers = {} ;
net{1}.layers{end + 1} = struct('type', 'mlp', 'weights', {{f * randn(n_hidden_nodes, n_hidden_nodes + n_input_nodes, 'single'), zeros(n_hidden_nodes, 1, 'single')}}) ;
net{1}.layers{end + 1} = struct('type', 'tanh') ;

net{2}.type = 'Fit';
net{2}.layers = {};
net{2}.layers{end + 1} = struct('type', 'mlp', 'weights', {{f * randn(n_output_nodes, n_hidden_nodes, 'single'), zeros(n_output_nodes, 1, 'single')}});
