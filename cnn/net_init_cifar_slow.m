...
% block 2
net.layers{end + 1} = struct('type', 'conv', 'weights', {{f * randn(5, 5, 32, 32, 'single'), zeros(1, 32, 'single')}}, 'stride', 1, 'pad', 2);
net.layers{end + 1} = struct('type', 'relu');
net.layers{end + 1} = struct('type', 'pool', 'pool', 3, 'stride', 2, 'pad', [0, 1, 0, 1]);

% block 3
net.layers{end + 1} = struct('type', 'conv', 'weights', {{f * randn(5, 5, 32, 64, 'single'), zeros(1, 64, 'single')}}, 'stride', 1, 'pad', 2);
net.layers{end + 1} = struct('type', 'relu');
net.layers{end + 1} = struct('type', 'pool', 'pool', 3, 'stride', 2, 'pad', [0, 1, 0, 1]);

% block 4
net.layers{end + 1} = struct('type', 'conv', 'weights', {{f * randn(4, 4, 64, 64, 'single'), zeros(1, 64, 'single')}}, 'stride', 1, 'pad', 0);
net.layers{end + 1} = struct('type', 'relu');

% block 5
net.layers{end + 1} = struct('type', 'conv', 'weights', {{f * randn(1, 1, 64, 10, 'single'), zeros(1, 10, 'single')}}, 'stride', 1, 'pad', 0);
