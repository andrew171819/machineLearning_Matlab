...
if isempty(dzdy)
    % forward mode, compute valid convolution using fft
    if (~isempty(pad))
        I = pad_data(I, pad, []);
    end
    ...
    opts.layer{opts.current_layer}.fI = fft2(I);
    opts.layer{opts.current_layer}.fk = fft2(kernel);
    ...
    % back prop, load the precomputed ffts and proceed with the computation
    % calculate valid correlation + flipping
    ...
    if ~isempty(bias)
        ...
        % minibatch averaging + patch summing
